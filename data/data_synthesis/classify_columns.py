from data.data_synthesis.recategorize_ontologically import build_mapping_table
...
def classify_columns(conn, table_classes):
    ...
    for table_name, info in table_classes.items():
        ...
        # leaf_header = info["leaf_header"]

        # 2. Assign known non-raw-col values (from classify_tables)
        '''
        These come from earlier pipeline stages:
          - country
          - ancestral_class (equipment_class)
          - sub_class (leaf ontological class)
        '''

        contextual_mapping[table_name]["country"].append(info["country"])
        contextual_mapping[table_name]["ancestral_class"].append(info["ancestral_class"])
        contextual_mapping[table_name]["sub_class"].append(info["equipment_class"])
        ...
        Examples:
          ...
          - "status" → status
          - "notes" → note
          - "section" → sub_class (merged header rows)
        ...
        for raw_col in raw_cols:
            if raw_col.lower() == "section":
                contextual_mapping[table_name]["sub_class"].append(raw_col)
                continue

            # TODO: implement lexical matching rules
            # super_col = map_raw_col_to_super_col(raw_col, eq_class, other_classes, canonical_columns, ontology_columns)
            # contextual_mapping[table_name][super_col].append(raw_col)
            pass
